...
# 2D DCPD thermo-chemical model, with heat convection on top surface
...
## Heat convection ##
h_conv = 50         # heat convection coefficient [W/m^2K]
...
F_T = (kr*dot(grad(w),grad(T)) + rho*Cp*w*(T - T_n)/dt - rho*Hr*w*(alpha_ - alpha_n)/dt)*dx + w*h_conv*(T - T_amb)*ds(4)
# NOTE: boundary integral on top surface for heat convection
...
# Output file
file_results = XDMFFile("results_2D_DCPD_convection.xdmf")
